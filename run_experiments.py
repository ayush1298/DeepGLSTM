...
def main(args):
    ...
    for d in datasets_to_run:
        run_cmd(f"python3 training.py --dataset {d} --epoch {epochs} {subset_flag} {attn_flag} --save_file base_{d}{attn_suffix} --model DeepGLSTM")

    if args.use_esm:
        print("\n=== Experiment 1.5: Compare performance with ESM_GCN model ===")
        # 2. ESM_GCN model (default frozen)
        for d in datasets_to_run:
            run_cmd(f"python3 training.py --dataset {d} --epoch {epochs} {subset_flag} {attn_flag} --save_file esmgcn_frozen_{d}{attn_suffix} --model ESM_GCN --freeze_esm")
        ...
        for d in datasets_to_run:
            if d == 'davis': # typically we only want to finetune on one large dataset as it's expensive
                run_cmd(f"python3 training.py --dataset {d} --epoch {epochs} {subset_flag} {attn_flag} --save_file esmgcn_finetune_{d}{attn_suffix} --model ESM_GCN")
        ...
        if 'davis' in datasets_to_run and 'kiba' in datasets_to_run:
            run_cmd(f"python3 inference.py --dataset kiba --model ESM_GCN --freeze_esm --load_model pretrained_model/esmgcn_frozen_davis{attn_suffix}.model {subset_flag} {attn_flag}")
